...
This turned out to be very controversial, sparking conversations on Twitter and Reddit. It seems that the majority view (or at least, the most vocally expressed view) is that one should *use* `const` *wherever possible,* only falling back to `js^let` where necessary, as can be enforced with the [prefer-const](https://eslint.org/docs/rules/prefer-const) ESLint rule.
